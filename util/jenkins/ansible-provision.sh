...
ansible -c ssh -i "${deploy_host}," $deploy_host -m shell -a "sudo /edx/bin/supervisorctl restart edxapp_worker:lms_default_1  && sudo /edx/bin/supervisorctl restart edxapp_worker:lms_high_1 && sudo /edx/bin/supervisorctl restart edxapp_worker:lms_high_mem_1 && sudo /edx/bin/supervisorctl restart edxapp_worker:cms_default_1  && sudo /edx/bin/supervisorctl restart edxapp_worker:cms_high_1" -u ubuntu -b
