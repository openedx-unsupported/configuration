...
#
# this script has been modified to upload files to google cloud storage 
#	requires:
...
#		then run:
#			gsutil config -e
#		with service account creds to generate ~/.boto file
#
#  CMD to run in cron:
# 	sudo crontab -e
# 		#Open edX DB Backups 3AM EST Daily (tjk)
# 		0 8 * * * /edx/bin/edx_db_backup_gcloud.py backup
#
...
GCLOUD_BUCKET = os.environ.get('EDX_BACKUPS_BUCKET','bucket-name')
...
def upload_to_gcloud_storage(file_name):
    GOOGLE_STORAGE = 'gs'
    gcloud_uri = boto.storage_uri(GCLOUD_BUCKET + '/' + file_name, GOOGLE_STORAGE)

    fp = open(os.path.join(BACKUPS_FOLDER, file_name), 'rb')
    gcloud_uri.new_key().set_contents_from_file(fp)
...
if __name__ == '__main__':
    if sys.argv[1] == "backup":
        ...
        upload_to_gcloud_storage(mongo_dump)
        clean_up(mongo_dump)
        mysql_dump = dump_service('mysql')
        upload_to_gcloud_storage(mysql_dump)
        clean_up(mysql_dump)
